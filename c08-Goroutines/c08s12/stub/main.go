...
// TODO - add paramter to wordCounter to accept another channel of []string, similar to 'wordsCh0'
func wordCounter(result *map[string]int, wordsCh0 <-chan []string, wg *sync.WaitGroup) {
	inCh := make(chan []string)

	// we need to know when there are no more words from 'wordsCh0' and 'wordsCh1', so we create our own wait WaitGroup
	var myWg sync.WaitGroup

	// the next goroutines reads from 'wordsCh0' to 'inCh'
	myWg.Add(1)
	go func() {
	...
	// TODO - use the example above at lines 60-66 to fan-in from 'wordsCh1' to 'inCh'
