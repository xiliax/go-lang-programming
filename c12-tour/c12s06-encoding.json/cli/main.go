...
var (
	inDataFile  = "data.csv"
	outDataFile = "data.json"
)

// encode Person object to JSON
func main() {
	fmt.Println("Using encoding/json standard package")

	personCh := readCSVData(inDataFile)

	done, err := writeJSONData(outDataFile, personCh)
	if nil != err {
		log.Fatalln(err)
	}
...
func writeJSONData(filename string, in <-chan *Person) (done <-chan bool, err error) {
